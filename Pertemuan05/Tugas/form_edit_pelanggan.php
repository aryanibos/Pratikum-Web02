...
<form method="POST" action="proses_pelanggan.php">
    ...
    <div class="form-group row">
        ...
        <div class="col-8">
            <div class="input-group">
                ...
                <input id="jk" name="jk" type="radio" value="L" class="form-control" <?php if ($row['jk'] == 'L') echo 'checked' ?>> Laki - Laki
                <input id="jk" name="jk" type="radio" value="P" class="form-control" <?php if ($row['jk'] == 'P') echo 'checked' ?>> Perempuan
            </div>
        </div>
    </div>
    <div class="form-group row">
    ...
    <div class="form-group row">
        ...
        <div class="col-8">
            <?php
            $sqljenis = "SELECT * FROM pelanggan INNER JOIN kartu ON kartu.id = pelanggan.kartu_id WHERE pelanggan.id = $_idedit";
            ...
            <select id="kartu_id" name="kartu_id" class="custom-select">
                ...
                ?>
                    <?php if ($rowjenis['kartu_id'] == $rowjenis['id']) { ?>
                        <option value="<?= $rowjenis['id'] ?>" selected><?= $rowjenis['nama'] ?>
                        </option>
                    <?php } else { ?>
                        <option value="<?= $rowjenis['id'] ?>"><?= $rowjenis['nama'] ?>
                        </option>
                <?php
                    }
                }
